...
class CCBPGraph {
    ...
    */
	private function Init() {
		...
		$ytitle = new JPText(CCBPGraph::YTitle,floor($lm*.75),($height-$tm-$bm)/2+$tm);
		#$ytitle->SetFont(FF_VERA,FS_BOLD,$labelsize+1);
		$ytitle->SetAlign('right','center');
		$ytitle->SetAngle(90);
		$graph->Add($ytitle);
		
		$xtitle = new JPText(CCBPGraph::XTitle,($width-$lm-$rm)/2+$lm,$height - 10);
		...
		$time = new JPText(date($df),$width-10,$height-10);
